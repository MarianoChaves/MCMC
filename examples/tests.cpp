...
int main() {
    ...
    if (true) {
        ...
        nu::Mcmc my_sample(nwalkers, ndim, file_name);
    ...
    if (false) {
        ...
        // Initialize mcmc
        nu::Mcmc my_first_sample(nwalkers, ndim, init_pos);

        // Get the final chain, in that case only the initial beacuse we have
        // not runned
    ...
    if (false) {
        ...
        std::cout << "First Walker: " << test1.getPos()[0] << ", "
                  << test1.getPos()[1] << ", " << test1.getPos()[2] << "\n";
        std::cout << "First Walker when the First was dropped: "
                  << test2.getPos()[0] << ", " << test2.getPos()[1] << ", "
                  << test2.getPos()[2] << "\n";
        std::cout << "Second Walker when the First was dropped: "
                  << test3.getPos()[0] << ", " << test3.getPos()[1] << ", "
                  << test3.getPos()[2] << "\n";
        ...
        std::cout
            << "Substituted the actual First Walker by the older first walker: "
            << test4.getPos()[0] << ", " << test4.getPos()[1] << ", "
            << test4.getPos()[2] << "\n";
    }
    ...
    if (false) {
        ...
        // nu::Walker my_first_walker(ndim); /* In case you prefer to begin with
        // a void walker  */
        ...
        // my_first_walker.setPos(first_position); /* In case you prefer to
        // begin with a void walker  */
        my_first_walker.setPos(second_position);
        my_first_walker.setPos(third_position);

        std::cout << "Steps:" << my_first_walker.getSteps() << "\n";

        std::cout << "Actual Position:" << my_first_walker.getPos()[0] << ", "
                  << my_first_walker.getPos()[1] << ", "
                  << my_first_walker.getPos()[2] << "\n";

        std::cout << "First Position:" << my_first_walker.getPos(1)[0] << ", "
                  << my_first_walker.getPos(1)[1] << ", "
                  << my_first_walker.getPos(1)[2] << "\n";

        std::cout << "Second Position:" << my_first_walker.getPos(2)[0] << ", "
                  << my_first_walker.getPos(2)[1] << ", "
                  << my_first_walker.getPos(2)[2] << "\n";
    };
}